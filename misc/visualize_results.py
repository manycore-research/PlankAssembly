...
def main():
    ...
    for name in tqdm(test_names[:300]):
        ...
        if not os.path.exists(os.path.join(args.log_path, 'pred_jsons', f'{name}.json')):
            continue

        names.append(name)

        with open(os.path.join(args.log_path, 'pred_jsons', f'{name}.json')) as f:
            results = json.load(f)

        mesh = build_mesh(results['prediction'], transparent=True)
        mesh.export(os.path.join(args.log_path, 'pred_mesh', f'{name}.glb'))

        mesh = build_mesh(results['groundtruth'], transparent=True)
        mesh.export(os.path.join(args.log_path, 'gt_mesh', f'{name}.glb'))

        with open(os.path.join(args.log_path, 'metrics.json')) as f:
            metrics = json.load(f)
    ...
    imagetable(columns,
        out_file=os.path.join(args.log_path, 'index.html'),
        imsize=(256, 256),
        sticky_header=True,
        sort_style='materialize',
        sortable=True,
        # model_viewer_opts={"orientation": "90deg -90deg -90deg"},
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log_path', type=str, default="lightning_logs/version_0",
                        help='results path.')
    args = parser.parse_args()

    os.makedirs(os.path.join(args.log_path, 'pred_mesh'), exist_ok=True)
    os.makedirs(os.path.join(args.log_path, 'gt_mesh'), exist_ok=True)
