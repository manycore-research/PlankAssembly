...
def parse_svg(name, view):
    # load svg
    paths, attributes = svg2paths(os.path.join(args.root, 'data', args.data_type, 'svgs', f'{name}_{view}.svg'))
...
def prepare_annotation(name):
    ...
    with open(os.path.join(args.root, 'data', args.data_type, 'infos', f'{name}.json'), 'w') as f:
        json.dump({
            "name": name,
            # 2D inputs
            "lines": lines,
...
if __name__ == '__main__':
    ...
    os.makedirs(os.path.join(args.root, 'data', args.data_type, 'infos'), exist_ok=True)
